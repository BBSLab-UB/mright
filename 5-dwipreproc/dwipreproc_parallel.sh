...
# bash dwi_preproc.sh -o FOLDER_OUT -i BIDS_DIRECTORY -l LIST -s SESSION -p PCORES

echo SCRIPT FOR DWI PREPROCESSING
...
FSLSUB_PARALLEL=${PCORES}

today_date=$(date '+%Y%m%d')
echo 'Log file will be stored in the current directory.'
bash dwipreproc_parallel_aux.sh
'''#Slurm batch for parallelization
sbatch <<EOF
#!/bin/bash
...
srun bash dwipreproc_parallel_aux.sh
EOF'''
